...
func TestInstallScriptUsesInstallTokenAndImmutableConfig(t *testing.T) {
	st, mux := newTestServer(t, Options{DiscoveryKey: "discover", AdminToken: "admin-token", PublicURL: "https://host.example.com"})
	...
	req := httptest.NewRequest(http.MethodGet, "/api/agents/agent-1/install.sh?token=install-token", nil)
...
func TestInstallPowerShellUsesInstallTokenAndImmutableConfig(t *testing.T) {
	...
	req := httptest.NewRequest(http.MethodGet, "/api/agents/agent-1/install.ps1?token=install-token", nil)
	rr := httptest.NewRecorder()
	mux.ServeHTTP(rr, req)
	if rr.Code != http.StatusOK {
		t.Fatalf("install script status = %d, want 200: %s", rr.Code, rr.Body.String())
